...
public class GvrReticlePointer : GvrBasePointer
{
    ...
    public const float RETICLE_MIN_OUTER_ANGLE = 0f;
    // Angle at which to expand the reticle when intersecting with an object
    // (in degrees).
    public const float RETICLE_GROWTH_ANGLE = 2.5f;

    // Minimum distance of the reticle (in meters).
    public const float RETICLE_DISTANCE_MIN = 1f;
    // Maximum distance of the reticle (in meters).
    public const float RETICLE_DISTANCE_MAX = 1000f;
    ...
    {
        ReticleDistanceInMeters = Mathf.Clamp(ReticleDistanceInMeters, RETICLE_DISTANCE_MIN, RETICLE_DISTANCE_MAX);
